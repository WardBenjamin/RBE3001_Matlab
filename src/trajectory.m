classdef trajectory < handle
    ...
    methods
        ...
        function [n, nextPoint] = getNextSetpoint(trajectories)
            for n = 1:length(trajectories.Setpoints)
                if ~(trajectories.Setpoints(n).HasExecuted)
                    nextPoint = trajectories.Setpoints(n);
                    return
                end
            end
            n = length(trajectories.Setpoints) + 1;
            nextPoint = trajectories.Setpoints(end);
        end
    ...
    methods (Static)
        function [n, nextTraj] = getNextTrajectory(objs)
            for n = 1:length(objs)
                if ~(objs(n).HasFinished())
                    nextTraj = objs(n);
                    return
                end
            end
