function [worldCoord, size] = getObjLocation(cam, cameraParams, T_base_check, T_cam_check)
...
yMask = thresholdYellow(image, imageLab);
gMask = thresholdGreen(image, imageLab);
bMask = thresholdBlue(image, imageLab);
kMask = thresholdBlack(imageGrey);
